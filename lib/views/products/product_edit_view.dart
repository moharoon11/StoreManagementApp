...
class ProductEditView extends StatefulWidget {
  ...
  const ProductEditView({Key? key, required this.product, required this.categories}) : super(key: key);
...
class _ProductEditViewState extends State<ProductEditView> {
  ...
  Future<void> _updateProduct() async {
    ...
    try {
      final res = await ApiService.put('${ApiConfig.products}/${widget.product['id']}', body);
      if (res['success'] == true) {
        if (mounted) {
  ...
  Future<void> _deleteProduct() async {
    ...
    setState(() => _isLoading = true);
    try {
      final res = await ApiService.delete('${ApiConfig.products}/${widget.product['id']}');
      if (res['success'] == true) {
        if (mounted) {
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Center(
                  child: GestureDetector(
                      ...
                      showModalBottomSheet(
                        ...
                        builder: (_) => Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ListTile(
                                leading: const Icon(Icons.photo_library, color: Color(0xFF365FF4)),
                              ...
                              ListTile(
                                leading: const Icon(Icons.camera_alt, color: Color(0xFF365FF4)),
                    ...
                    child: Container(
                      ...
                      child: _isUploading
                          ? const Center(child: CircularProgressIndicator(color: Color(0xFF365FF4)))
                          : _pickedImageFile != null
                              ? Image.file(_pickedImageFile!, width: double.infinity, height: double.infinity, fit: BoxFit.contain)
                              : _productImageUrl.isNotEmpty
                                  ? Image.network(_productImageUrl, width: double.infinity, height: double.infinity, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const _ImagePlaceholder())
                ...
                Row(
                  children: [
                    Expanded(child: _buildTextField('Cost Price (₹)', _costPriceController, isNumber: true)),
                    const SizedBox(width: 16),
                    Expanded(child: _buildTextField('Selling Price (₹) *', _sellingPriceController, isNumber: true)),
                  ],
                ),
                const SizedBox(height: 16),
...
class _ImagePlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Column(
        ...
        children: [
          ...
          SizedBox(height: 4),
          Text('Upload Product Image', style: TextStyle(color: Color(0xFF6C7486), fontSize: 12, fontWeight: FontWeight.w500)),
        ],
      );
}
