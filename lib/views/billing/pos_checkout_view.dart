    ...
    try {
            ...
            title: SingleChildScrollView(
              ...
              child: Row(
                children: const [
                  ...
                  SizedBox(width: 8),
                  Text('Checkout Completed', style: TextStyle(color: Color(0xFF0F172A), fontWeight: FontWeight.bold, fontSize: 18)),
                ],
              ),
            ),
            content: Column(
              ...
              children: [
                Text('Invoice #: ${invoice['invoiceNumber']}', style: const TextStyle(color: Color(0xFF0F172A), fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('Grand Total:', style: TextStyle(color: Color(0xFF64748B), fontWeight: FontWeight.w600)),
                    Text('₹${invoice['grandTotal']}', style: const TextStyle(color: Color(0xFF10B981), fontSize: 20, fontWeight: FontWeight.bold)),
                  ],
                ),
                const SizedBox(height: 8),
                Text('${(invoice['items'] as List?)?.length ?? 0} item types processed.', style: const TextStyle(color: Color(0xFF64748B), fontSize: 13)),
                ...
                Row(
                  ...
                  children: [
                    // PDF Download / Print
                    IconButton(
                      icon: const Icon(Icons.picture_as_pdf, color: Color(0xFFEF4444), size: 28),
                      tooltip: 'Download PDF',
                      onPressed: () async {
                        try {
                          final res = await http.get(Uri.parse(pdfUrl));
                          if (res.statusCode == 200) {
                            await Printing.layoutPdf(onLayout: (format) async => res.bodyBytes, name: 'Invoice_${invoice['invoiceNumber']}.pdf');
                          }
                        } catch (e) {
                          if (mounted) {
                    ...
                    // WhatsApp Share
                    IconButton(
                      icon: const Icon(Icons.share, color: Color(0xFF25D366), size: 28),
                      tooltip: 'Share on WhatsApp',
                      onPressed: () async {
                        final text = Uri.encodeComponent("Invoice #${invoice['invoiceNumber']} - Total: ₹${invoice['grandTotal']}\nPDF: $pdfUrl");
                    ...
                    // Print Invoice
                    IconButton(
                      icon: const Icon(Icons.print, color: Color(0xFF2563EB), size: 28),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Checkout Error: ${e.toString().replaceAll('Exception: ', '')}'), backgroundColor: const Color(0xFFEF4444)),
        );
      }
    }
              ...
              child: _isLoading
                  ...
                  : GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isInCart = provider.cartItems.containsKey(p['id']);
                        ...
                        return InkWell(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(12),
                              border: Border.all(
                                color: isInCart ? const Color(0xFF2563EB) : const Color(0xFFE2E8F0),
                                width: isInCart ? 2 : 1,
                              ),
                              boxShadow: [
                                BoxShadow(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const Spacer(),
                                Text('₹${p['sellingPrice']}', style: const TextStyle(color: Color(0xFF10B981), fontWeight: FontWeight.bold, fontSize: 14)),
                                const SizedBox(height: 4),
                                Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                  children: [
                                    Text('Stock: $stock', style: TextStyle(color: stock > 0 ? const Color(0xFF64748B) : const Color(0xFFEF4444), fontSize: 11)),
                                    const Icon(Icons.add_shopping_cart, color: Color(0xFF2563EB), size: 18),
                                  ],
                                ),
                              ],
                            ),
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Current Invoice', style: TextStyle(color: Color(0xFF0F172A), fontWeight: FontWeight.bold, fontSize: 16)),
                  IconButton(
                    ...
                    ? const Center(
                        child: Column(
                          ...
                          children: const [
                            ...
                            SizedBox(height: 8),
                            Text('Tap products to add to bill.', style: TextStyle(color: Color(0xFF64748B), fontSize: 13)),
                          ],
                        ),
                      )
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          final item = provider.cartItems.values.toList()[index];
                          final product = item['product'];
                          final qty = item['quantity'] as int;
                          final availableStock = (product['stockQuantity'] as int? ?? 9999);
                                  ...
                                  children: [
                                    InkWell(
                                      ...
                                      child: Container(
                                        ...
                                        child: const Icon(Icons.remove, color: Color(0xFFD97706), size: 16),
                                      ),
                                    ),
                                    Padding(
                                    ...
                                    InkWell(
                                      onTap: qty < availableStock
                                          ...
                                          : () {
                                              ScaffoldMessenger.of(context).showSnackBar(
                                                SnackBar(
                                                  content: Text('Cannot add more. Max stock available: $availableStock'),
                                      ...
                                      child: Container(
                                        padding: const EdgeInsets.all(4),
                                        decoration: BoxDecoration(
                                          color: qty < availableStock ? const Color(0xFFDBEAFE) : const Color(0xFFF1F5F9),
                                          shape: BoxShape.circle,
                                        ),
                                        child: Icon(
                                          Icons.add,
                                          color: qty < availableStock ? const Color(0xFF2563EB) : const Color(0xFF94A3B8),
                                          size: 16,
                                        ),
                                      ),
                                    ),
              ...
              Row(
                ...
                children: [
                  ...
                  Text('₹${provider.cartTotal.toStringAsFixed(2)}', style: const TextStyle(color: Color(0xFF10B981), fontSize: 20, fontWeight: FontWeight.bold)),
                ],
              ),
              const SizedBox(height: 14),
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: _isCheckingOut
                      ...
                      : Text('Checkout (${provider.cartCount} items)', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
